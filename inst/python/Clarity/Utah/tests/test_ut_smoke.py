"""
Smoke tests for the Utah election results scraper.
...
Run from ``inst/python/`` with::
    ...
    python -m Utah.tests.test_ut_smoke
    python -m Utah.tests.test_ut_smoke --year 2024
    python -m Utah.tests.test_ut_smoke --discovery-only
    python -m Utah.tests.test_ut_smoke --state-only   # skip county scraping
    python -m Utah.tests.test_ut_smoke --vote-methods # include vote-method breakdown
"""
...
def test_discovery() -> bool:
    ...
    print("\n[test_discovery] Rendering Utah elections landing page...")
    from Clarity.Utah.client import UtPlaywrightClient
    from Clarity.Utah.discovery import parse_election_links

    with UtPlaywrightClient() as client:
    ...
    if not elections:
        ...
        print("       with open('/tmp/ut_landing.html', 'w') as f: f.write(html)")
        return False
...
def test_single_election(year: int, level: str = "all") -> bool:
    ...
    from Clarity.Utah.client import UtPlaywrightClient
    from Clarity.Utah.discovery import parse_election_links
    from Clarity.Utah.parser import parse_state_results, parse_county_results, county_name_from_url
    ...
    state_df, vm_df, county_urls = parse_state_results(election_html, target)

    if state_df.empty:
        _fail(f"State parser returned empty DataFrame for '{target.name}'.")
        print("  TIP: with open('/tmp/ut_election.html', 'w') as f: f.write(election_html)")
        return False
    ...
    county_df, _ = parse_county_results(county_html, county, target, url=sample_url)
...
def test_pipeline_year_range(
    ...
    result = get_ut_election_results(
        year_from=year_from,
        year_to=year_to,
        level=level,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Smoke tests for the Utah election results scraper."
    )
    parser.add_argument(
