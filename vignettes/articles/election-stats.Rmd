...
New Hampshire features alongside Indiana in the cross-state section of
the [Example analyses vignette](https://gchickering21.github.io/DownBallotR/articles/example-analyses.html),
which shows how to combine multiple ElectionStats and non-ElectionStats
states with `bind_rows()` for side-by-side comparison.
...
| `level` | `"all"` | Constituency level of the returned results — see below |
| `parallel` | `TRUE` | Parallel county scraping (classic states only) |

### The `level` parameter (constituency level)

`level` selects which **constituency-level** data frame(s) to return — i.e.,
the geographic unit at which votes are tabulated. It is **not** the same as
office level (Federal / State / Local), which appears in the `office_level`
column.

| Value | Constituency | Returns |
|---|---|---|
| `"all"` (default) | All available | Named list with `$state`, `$county`, and (where available) `$precinct` data frames |
| `"state"` | State-level constituency | Statewide candidate results data frame |
| `"county"` | County-level constituency | County vote breakdown data frame |
| `"precinct"` | Precinct-level constituency | Precinct vote breakdown data frame (CO, MA, ID, SC, NM, VA) |
