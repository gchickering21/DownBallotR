...
Indiana is the running example in the
[Example analyses vignette](https://gchickering21.github.io/DownBallotR/articles/example-analyses.html),
which demonstrates filtering by office, counting party wins, margin
calculations, and county-level swing analysis.
...
state_df %>%
  select(election_year, office_level, office, candidate, party, votes, vote_pct, winner)
```
...
res$county %>%
  ...
  select(office, candidate, party, votes, vote_pct) %>%
...
| `candidate` | Candidate name as printed on the ballot |
